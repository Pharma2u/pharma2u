  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // A temporary API outage should not crash the rider workspace.
      }
    };
    void load();
  ...
  return (
    <div className="relative">
      ...
      >
        <Bell size={19} />
        {items.length > 0 && (
          <i className="absolute right-2 top-2 h-2 w-2 rounded-full bg-emerald-500 ring-2 ring-white" />
        )}
      </button>
      {open && (
        <div className="fixed inset-x-3 top-16 z-50 max-h-[70vh] overflow-auto rounded-2xl border border-slate-200 bg-white p-4 shadow-2xl sm:absolute sm:inset-auto sm:right-0 sm:top-12 sm:w-80">
