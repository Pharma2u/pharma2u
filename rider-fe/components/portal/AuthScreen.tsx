    ...
    <main className="min-h-screen bg-slate-50 lg:grid lg:grid-cols-[0.9fr_1.1fr]">
      <section className="relative overflow-hidden bg-slate-950 px-6 py-8 text-white sm:px-10 lg:flex lg:min-h-screen lg:flex-col lg:justify-between lg:p-14">
        ...
        <CompanyLogo
          width={230}
          height={86}
          className="relative h-[86px] w-[230px] brightness-0 invert"
          priority
        />
        <div className="relative mt-12 max-w-xl lg:my-auto">
          ...
          <h1 className="mt-5 text-4xl font-bold leading-tight tracking-tight sm:text-5xl">
            Deliver healthcare.
            <br />
            <span className="text-emerald-400">Earn with clarity.</span>
          </h1>
          <p className="mt-5 max-w-lg text-sm leading-7 text-slate-300 sm:text-base">
            A secure rider workspace for verified deliveries, live navigation,
            and transparent settlement accounting.
          </p>
          <div className="mt-8 grid gap-3 sm:grid-cols-3">
            {[
              [ShieldCheck, "Verified onboarding"],
              [WalletCards, "Clear earnings"],
              [CheckCircle2, "Protected data"],
            ].map(([Icon, label]) => {
              const C = Icon as typeof ShieldCheck;
              return (
                <div
                  key={String(label)}
                  className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 p-3 text-xs font-semibold"
                >
                  <C size={17} className="text-emerald-400" />
                  {String(label)}
                </div>
              );
            })}
        ...
        <p className="relative mt-10 text-xs text-slate-500">
          Sensitive customer details remain protected at every step.
        </p>
      </section>
          ...
          {mode === "login" ? (
            <section className="mx-auto max-w-md rounded-3xl border border-slate-200 bg-white p-6 shadow-xl shadow-slate-950/5 sm:p-8">
              ...
              <p className="mt-2 mb-6 text-sm text-slate-500">
                Use your password or a secure one-time code.
              </p>
              <RiderLoginForm onAuthenticated={onAuthenticated} />
            </section>
          ) : (
            <ApplicationForm />
          )}
