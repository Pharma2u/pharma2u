...
export function getDestination(task: RiderTask) {
  if (task.dropLat != null && task.dropLng != null) return `${task.dropLat},${task.dropLng}`;
...
export function taskStatusLabel(task: RiderTask) {
  if (task.leg === 'relay') return task.status === 'relay_pending' ? 'Handoff ready' : 'Proceed to relay point';
  if (task.status === 'rider_assigned') return 'Pickup assigned';
  if (task.status === 'picked_up' || task.status === 'relay_pending') return 'Pickup complete';
  if (task.status === 'on_the_way') return 'On the way';
  return task.status.replaceAll('_', ' ');
}

export function nextTaskAction(task: RiderTask) {
  if (task.leg === 'relay') return task.status === 'relay_pending' ? 'Confirm handoff complete' : null;
  if (task.status === 'rider_assigned') return 'Verify pickup';
  if (task.status === 'picked_up' || task.status === 'relay_pending') return 'Start delivery';
  if (task.status === 'on_the_way') return 'Verify and complete delivery';
  return null;
}
