...
import type { PublicPharmacy } from "@/src/lib/pharmacy";
...
export default function SearchResults({
  ...
  const [deliveryFilter, setDeliveryFilter] = useState<"all" | "10" | "15">(
    "all",
  );

  const [discountFilter, setDiscountFilter] = useState<"all" | "10" | "20">(
    "all",
  );
  ...
  const filteredProducts = useMemo(() => {
    let results = products.filter((product) => {
      ...
      const matchesDelivery =
        deliveryFilter === "all" ||
        (deliveryFilter === "10" && deliveryMinutes <= 10) ||
        (deliveryFilter === "15" && deliveryMinutes <= 15);

      const matchesDiscount =
        discountFilter === "all" ||
        (discountFilter === "10" && product.discount >= 10) ||
        (discountFilter === "20" && product.discount >= 20);
    ...
    <>
      ...
      <div className="flex items-center justify-between border-b border-[#EDF0EF] px-5 py-4">
        <div className="flex items-center gap-2">
          <SlidersHorizontal size={18} className="text-[#2EB68F]" />

          <h2 className="text-sm font-bold text-[#17212B]">Filters</h2>
        </div>
      ...
      <div className="border-b border-[#EDF0EF] p-5">
        <h3 className="text-sm font-bold text-[#17212B]">Prescription</h3>
        ...
        <div className="mt-4 space-y-3">
          <label className="flex cursor-pointer items-center gap-3">
            ...
            <span className="text-sm text-[#64717D]">All products</span>
          </label>
      ...
      <div className="border-b border-[#EDF0EF] p-5">
        <h3 className="text-sm font-bold text-[#17212B]">Delivery time</h3>
          ...
          {[
            ...
            >
              <input
                ...
                onChange={() => setDeliveryFilter(value as "all" | "10" | "15")}
                className="h-4 w-4 accent-[#45C9A5]"
              />

              <span className="text-sm text-[#64717D]">{label}</span>
            </label>
          ))}
        </div>
      </div>
      ...
      <div className="p-5">
        <h3 className="text-sm font-bold text-[#17212B]">Discount</h3>
          ...
          {[
            ...
            >
              <input
                ...
                onChange={() => setDiscountFilter(value as "all" | "10" | "20")}
                className="h-4 w-4 accent-[#45C9A5]"
              />
    ...
    <>
      {matchingPharmacies.length > 0 && (
        <section className="mb-7">
          <p className="mb-3 text-sm font-bold text-[#17212B]">Pharmacies</p>
          <div className="flex gap-3 overflow-x-auto pb-2">
            {matchingPharmacies.map((pharmacy) => (
              <Link
                key={pharmacy.id}
                href={`/pharmacy/${pharmacy.id}`}
                className="min-w-56 rounded-xl border border-[#DDE5E2] bg-white p-4 text-sm font-bold text-[#17212B]"
              >
                {pharmacy.name}
                <span
                  className={`mt-2 block text-xs ${pharmacy.isOpen ? "text-[#109D70]" : "text-red-600"}`}
                >
                  {pharmacy.isOpen ? "Open" : "Offline"}
                </span>
                <span className="mt-1 block truncate text-xs font-normal text-[#64717D]">
                  {pharmacy.address}
                </span>
              </Link>
            ))}
          ...
          {filteredProducts.length > 0 ? (
            ...
            <div className="flex min-h-[420px] flex-col items-center justify-center rounded-2xl border border-[#E5EAE8] bg-white px-6 text-center">
              <div className="flex h-20 w-20 items-center justify-center rounded-full bg-[#EAFAF5]">
                <SearchX size={34} className="text-[#2EB68F]" />
              </div>
