...
export function PasswordInput(props: InputHTMLAttributes<HTMLInputElement>) {
  ...
  return (
    <div className="relative mt-2">
      ...
      <button type="button" aria-label={visible ? "Hide password" : "Show password"} aria-pressed={visible} onClick={() => setVisible((value) => !value)} className="absolute inset-y-0 right-0 grid w-12 place-items-center text-slate-500 hover:text-slate-800">{visible ? <EyeOff /> : <Eye />}</button>
    </div>
  );
}
