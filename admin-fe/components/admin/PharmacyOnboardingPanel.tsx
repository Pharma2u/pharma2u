...
const onboardingSchema = z.object({
  ...
  address: z
    .string()
    .trim()
    .min(10, "Enter a complete address (at least 10 characters).")
    .max(200),
  drugLicenseNumber: z
    .string()
    .trim()
    .regex(/^[A-Za-z0-9/-]{5,40}$/, "Use 5–40 letters, numbers, / or -."),
  pharmacistName: z
    .string()
    .trim()
    .min(2, "Enter the pharmacist name.")
    .max(80),
  ...
  vendorPhone: z
    ...
    .regex(
      /^[6-9]\d{9}$/,
      "Use a 10-digit Indian mobile number, e.g. 9876543210.",
    ),
...
const fields: FieldConfig[] = [
  {
    key: "pharmacyName",
    label: "Pharmacy name",
    placeholder: "e.g. Pradeep Medicals",
  ...
  },
  {
    ...
    hint: "Include locality and city",
  },
  {
    key: "drugLicenseNumber",
    label: "Drug licence number",
    placeholder: "e.g. AP/12345/2026",
  ...
  },
  {
    ...
    hint: "Registered pharmacist",
  },
  {
    key: "pharmacistLicenseNumber",
  ...
  },
  {
    ...
    hint: "Vendor who will manage stock",
  },
  {
    key: "vendorPhone",
...
export function PharmacyOnboardingPanel({ token }: { token: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setNotice(
        `Pharmacy created. Vendor phone: ${created.vendorPhone}. Temporary password: ${created.temporaryPassword}`,
      );
      setValues(emptyValues());
    } catch (error) {
      setSubmitError(
        error instanceof Error ? error.message : "Unable to create pharmacy.",
      );
    } finally {
  ...
  return (
    <section className="mx-auto max-w-3xl rounded-3xl bg-white p-6 shadow-sm sm:p-7">
      ...
      <h2 className="mt-2 text-2xl font-bold text-slate-950">
        Create pharmacy and vendor
      </h2>
      <p className="mt-2 text-sm leading-6 text-slate-500">
        Enter the pharmacy licence details, its location, and the vendor account
        that will manage stock.
      </p>
      ...
      >
        <section className="grid gap-4 sm:col-span-2 sm:grid-cols-2">
          <h3 className="sm:col-span-2 text-sm font-bold text-slate-900">
            Pharmacy details
          </h3>
        ...
        {submitError && (
          <p
            role="alert"
            className="rounded-xl border border-red-100 bg-red-50 p-3 text-sm text-red-700 sm:col-span-2"
          >
            {submitError}
          </p>
        )}
        {notice && (
          <p
            role="status"
            className="rounded-xl border border-emerald-100 bg-emerald-50 p-3 text-sm text-emerald-800 sm:col-span-2"
          >
            {notice}
          </p>
        )}
        <button
          ...
          className="rounded-xl bg-emerald-500 px-5 py-3 font-semibold text-slate-950 transition hover:bg-emerald-400 disabled:cursor-not-allowed disabled:opacity-60 sm:col-span-2"
        >
