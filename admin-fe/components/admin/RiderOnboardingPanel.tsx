...
export function RiderOnboardingPanel({ token }: { token: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const rider = await provisionStaff(
        token,
        name,
        phone,
        email,
      ...
      setNotice(
        `Rider created. Phone: ${rider.phone}. Temporary password: ${rider.temporaryPassword}`,
      );
      setName("");
      setPhone("");
      setEmail("");
  ...
  return (
    <section className="mx-auto mt-8 max-w-2xl rounded-3xl bg-white p-7 shadow-sm">
      ...
      <p className="mt-2 text-sm text-slate-500">
        Create an approved rider directly. The rider must change their temporary
