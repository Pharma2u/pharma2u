...
export function ProductImage({
  ...
  if (image) {
    return (
      <Image
        ...
        height={large ? 56 : 48}
        alt=""
        className={`${size} shrink-0 rounded-xl border border-slate-200 bg-white object-cover`}
      />
    );
  }
  ...
  return (
    <div
      className={`${size} grid shrink-0 place-items-center rounded-xl bg-teal-100 text-sm font-bold text-teal-700`}
      aria-hidden="true"
    >
      ...
      <div className="flex min-w-0 items-center gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            {product.genericName} / {productCategoryLabels[product.category]} /
            INR {product.price.toFixed(2)} / {product.unit}
          </p>
          <p
            className={`mt-1 text-sm font-semibold ${low ? "text-amber-700" : "text-slate-700"}`}
          >
