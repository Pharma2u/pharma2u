...
import { AddProductForm, type ProductDraft } from "./AddProductForm";
import {
  ProductImage,
  ProductImageGallery,
  ProductRow,
...
export function InventoryPanel({
  ...
  const pharmacyOnlyView = showPharmacyProfile && !showCatalogue;
  ...
  }, [token, search, category, pharmacyOnlyView]);
  useEffect(() => {
    if (!notice) return;
  ...
  async function savePharmacyProfile(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const updated = await updateMyPharmacyProfile(token, {
        name: String(form.get("pharmacyName")).trim(),
        address: String(form.get("pharmacyAddress")).trim(),
        ...
        logo:
          form.get("logo") instanceof File && (form.get("logo") as File).size
            ? (form.get("logo") as File)
            : undefined,
        banner:
          form.get("banner") instanceof File &&
          (form.get("banner") as File).size
            ? (form.get("banner") as File)
            : undefined,
      });
      setPharmacy(updated);
      setNotice("Pharmacy profile updated.");
    } catch (caught) {
      setError(
        caught instanceof Error
          ? caught.message
  ...
  function selectProductImages(
    ...
    const invalid = images.find(
      (image) =>
        !allowedTypes.includes(image.type) || image.size > 5 * 1024 * 1024,
    );
  ...
  async function add(event: FormEvent<HTMLFormElement>) {
    ...
    if (
      expiryDate &&
      new Date(`${expiryDate}T00:00:00`).getTime() <= Date.now()
    ) {
    ...
    try {
      await createProduct(token, {
        ...
        productType: String(form.get("productType") || "TABLET"),
        price,
  ...
  async function saveProduct(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const updated = await updateProduct(token, productToEdit.id, {
        ...
        category: String(form.get("category")) as ProductCategory,
        productType: String(
          form.get("productType") || productToEdit.productType || "TABLET",
        ),
  ...
  async function addProducts(drafts: ProductDraft[]) {
    setAdding(true);
    setError("");
    let completed = 0;
    try {
      for (const draft of drafts) {
        ...
        if (
          ...
          throw new Error(
            `Row ${completed + 1}: expiry date must be in the future.`,
          );
        await createProduct(token, {
          ...
          stock: Number(draft.stock) + Number(draft.freeStock || 0),
          freeStock: Number(draft.freeStock || 0),
          batchNumber: draft.batchNumber.trim() || undefined,
          expiryDate: expiryDate || undefined,
          unitsPerStrip: draft.unitsPerStrip
            ? Number(draft.unitsPerStrip)
            : undefined,
          stripsPerBox: draft.stripsPerBox
            ? Number(draft.stripsPerBox)
            : undefined,
          ...
          rackNumber: draft.rackNumber.trim() || undefined,
      ...
      setNotice(
        `${completed} product${completed === 1 ? "" : "s"} added to inventory.`,
      );
      await load();
      return completed;
    } catch (caught) {
      const reason =
        caught instanceof Error ? caught.message : "Unable to add products.";
      setError(
        completed
          ? `${completed} product(s) saved before an error: ${reason}`
          : reason,
      );
      return completed;
    } finally {
  ...
  const dedicatedAddProduct =
    startAdding && !showCatalogue && !showPharmacyProfile;
      ...
      {showPharmacyProfile && pharmacy && (
        <div className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-white px-5 py-4 shadow-sm">
          ...
          <button
            ...
            aria-label={
              pharmacy.isOpen
                ? "Close pharmacy for orders"
                : "Open pharmacy for orders"
            }
          ...
          >
            <span
              className={`inline-block h-6 w-6 rounded-full bg-white shadow-sm transition-transform ${pharmacy.isOpen ? "translate-x-7" : "translate-x-1"}`}
            />
            <span className="sr-only">
              {updatingPharmacyStatus
                ? "Updating pharmacy status"
      ...
      {showPharmacyProfile && pharmacy && (
        ...
        >
          <div className="md:col-span-2">
            ...
            <h2 className="mt-1 text-lg font-bold text-slate-900">
              Store details customers will see
            </h2>
          </div>
          ...
          <label className="grid gap-1 text-sm font-semibold text-slate-700">
            ...
            <input
              ...
              required
              type="time"
              defaultValue={pharmacy.openingTime ?? "09:00"}
              className="rounded-xl border border-slate-200 px-3 py-2.5 font-normal"
            />
          </label>
          <label className="grid gap-1 text-sm font-semibold text-slate-700">
            ...
            <input
              ...
              required
              type="time"
              defaultValue={pharmacy.closingTime ?? "21:00"}
              className="rounded-xl border border-slate-200 px-3 py-2.5 font-normal"
            />
          </label>
          <label className="grid gap-1 text-sm font-semibold text-slate-700">
          ...
          <label className="grid gap-1 text-sm font-semibold text-slate-700">
            ...
            {pharmacy.bannerPath && (
              <Image
                unoptimized
                src={pharmacy.bannerPath}
                alt="Current pharmacy cover"
                width={800}
                height={160}
              ...
              {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((day) => (
                ...
                >
                  <input
                    ...
                    value={day}
                    defaultChecked={
                      pharmacy.operatingDays?.includes(day) ??
          ...
          <div className="flex justify-end md:col-span-2">
            ...
            >
              {savingProfile ? "Saving..." : "Save pharmacy profile"}
            </button>
          </div>
        </form>
      )}{" "}
              ...
              >
                <div className="sm:col-span-2 lg:col-span-3 rounded-2xl border border-teal-100 bg-teal-50/70 px-4 py-3">
                  ...
                  <p className="mt-1 text-sm text-teal-800">
                    Add the medicine, selling price and opening stock first.
                    Batch, expiry, images and customer-facing details can be
                ...
                <label className="grid gap-1.5 text-sm font-semibold text-slate-700">
                  ...
                  <input
                    ...
                    accept="image/jpeg,image/png,image/webp"
                    onChange={(event) =>
                      selectProductImages(event, setNewProductImages)
                    }
                  ...
                  {newProductImages.length > 0 && (
                    <span className="rounded-lg bg-teal-50 px-2.5 py-2 text-xs font-semibold leading-5 text-teal-800">
                      {newProductImages.length} image
                      {newProductImages.length === 1 ? "" : "s"} selected:{" "}
                      {newProductImages.map((image) => image.name).join(", ")}
                    </span>
                  )}
                </label>
                <InputField
                    ...
                    <div className="mb-3 flex items-center justify-between gap-3 px-1">
                      <div>
                        ...
                        <p className="text-sm text-slate-500">
                          Select a product to update its details or stock.
                        </p>
                      </div>
          ...
          <form onSubmit={saveProduct} className="grid gap-4">
            <div className="flex items-center gap-3 rounded-xl border border-teal-100 bg-teal-50/60 p-3">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-0.5 text-sm text-slate-600">
                  {productToEdit.genericName} / {labels[productToEdit.category]}
                </p>
                <p className="mt-1 text-xs font-semibold text-teal-700">
                  Current stock: {productToEdit.stock} {productToEdit.unit}
                </p>
              </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <label className="grid gap-1.5 text-sm font-semibold text-slate-700">
                ...
                <select
                  name="productType"
                  defaultValue={productToEdit.productType || "TABLET"}
                  className="rounded-xl border border-slate-200 bg-white px-3 py-2.5"
                >
                  {productTypes.map((type) => (
                    <option key={type}>{type}</option>
            ...
            <label className="grid gap-1.5 text-sm font-semibold text-slate-700">
              ...
              <input
                ...
                accept="image/jpeg,image/png,image/webp"
                onChange={(event) =>
                  selectProductImages(event, setEditProductImages)
                }
                className="rounded-xl border border-slate-200 px-3 py-2.5 font-normal"
              />
              <span className="text-xs font-normal leading-5 text-slate-500">
                Leave empty to keep the current gallery. Choosing images
                replaces the full gallery.
              </span>
              {editProductImages.length > 0 && (
                <span className="rounded-lg bg-teal-50 px-2.5 py-2 text-xs font-semibold leading-5 text-teal-800">
                  {editProductImages.length} replacement image
                  {editProductImages.length === 1 ? "" : "s"} selected:{" "}
                  {editProductImages.map((image) => image.name).join(", ")}
                </span>
              )}
            </label>
            <FormActions
